...
#endif
#ifdef CBC
void lp_config_cbc_params(LinearProgram *lp, string &cbcOpt);
void tuneclp_options( OsiClpSolverInterface &lp, ClpSolve &clpOptions );
#endif
#ifdef CPX
...
#ifdef CBC
    bool deleteLP = false;
    OsiClpSolverInterface *linearProgram = NULL;

    {
        lp->status = LP_ERROR;

        if (lp_isMIP(lp)) {
            linearProgram = dynamic_cast<OsiClpSolverInterface *>( lp->_lp->clone() );
            deleteLP = true;
        }
        else
            linearProgram = lp->_lp;

        ClpSimplex *clp = linearProgram->getModelPtr();
        clp->setPerturbation( 50 );

        if ( lp->maxSeconds != INT_NOT_SET )
            clp->setMaximumSeconds( lp->maxSeconds ); 

        if ( (!lp_isMIP(lp)) || (lp->optAsContinuous) )
        {
            ...
            {
                ClpSolve clpOptions;
                tuneclp_options( *linearProgram, clpOptions );
                linearProgram->setSolveOptions( clpOptions );
                linearProgram->initialSolve();
            }
            
            // checking result of LP optimization
            ...
            if (linearProgram->isProvenOptimal()) {
        ...
        } // not mip or optimizing as continuous
        else
        { // MIP
            //clock_t start = clock();
            ClpSolve clpOptions;
            ...
            linearProgram->initialSolve( );
            //clock_t end = clock();
            //double s = ((double)end-start) / (double)CLOCKS_PER_SEC;
            //printf("lp relax solved in %g seconds obj %g\n", s, linearProgram->getObjValue() );
...
#define STR_OPT_SIZE 256
                string cbcopt;
                lp_config_cbc_params( lp, cbcopt );

                callCbc( cbcopt, *model );
...
void lp_config_cbc_params(LinearProgram *lp, string &cbcopt )
{
    ...
    {
        cbcopt.append(" -cutoff");
        cbcopt.append("="+SSTR(lp->cutoff));
        if (lp->cutoffAsConstraint)
        {
            cbcopt.append( "-constraintfromCutoff" );
            cbcopt.append( "=on" );
        }
    }
    ...
    if ( lp->parallel != INT_NOT_SET )
    {
        if ( lp->parallel )
        {
            ...
            cbcopt.append(" -threads");
            cbcopt.append("="+SSTR(nthreads));
        }
        else
        {
            cbcopt.append( " -threads" );
    ...
    {
        cbcopt.append(" -timeM");
        cbcopt.append("=elapsed");
        cbcopt.append(" -seconds");
        cbcopt.append("="+SSTR(lp->maxSeconds));
    }
    if (lp->maxSolutions != INT_NOT_SET)
    {
        cbcopt.append(" -maxSol");
        cbcopt.append("="+SSTR(lp->maxSolutions));
    }
    if (lp->maxNodes != INT_NOT_SET)
    {
        cbcopt.append(" -maxNodes");
        cbcopt.append("="+SSTR(lp->maxNodes));
    }
    if (lp->heurFPPasses  != INT_NOT_SET)
    {
        cbcopt.append(" -passF");
        cbcopt.append("="+SSTR(lp->heurFPPasses));
    }
    if (lp->heurProx != INT_NOT_SET) {
        if (lp->heurProx)
        {
            cbcopt.append(" -proximity");
            cbcopt.append("=on");
        }
        else
        {
    ...
    {
        cbcopt.append(" -maxSaved");
        cbcopt.append(string("=")+SSTR(lp->maxSavedSols));
    }

    if (strlen(lp->solInFN))
    {
        cbcopt.append(" -mips");
        cbcopt.append(string("=")+lp->solInFN);
    }

    if (lp->absMIPGap!=DBL_MAX)
    {
        cbcopt.append(" -allowableGap");
        cbcopt.append( "="+SSTR(lp->absMIPGap) );
    }
    if (lp->relMIPGap!=DBL_MAX)
    {
        cbcopt.append(" -ratioGap");
        cbcopt.append("="+SSTR(lp->relMIPGap));
    }
    if ( lp->mipPreProcess != CHAR_MAX )
    {
        if (!lp->mipPreProcess)
        {
            cbcopt.append(" -preprocess");
            cbcopt.append(" off");
        }
    }
...
{
    int binaries = 0;
    for(int i = 0; i < lp_cols((LinearProgram *)lp); i++)
    {
        const double lb = lp_col_lb((LinearProgram *)lp, i);
        const double ub = lp_col_ub((LinearProgram *)lp, i);
        if( (lp_is_integer( (LinearProgram *)lp, i )) && ( fabs( lb ) <= EPS ) && (fabs( ub - 1.0 ) <= EPS) )
            binaries++;
    }
    return binaries;
}
...
void tuneclp_options( OsiClpSolverInterface &lp, ClpSolve &clpOptions )
{
    double minAij = DBL_MAX;
    double maxAij = -DBL_MAX;
    double minR = DBL_MAX;
    double maxR = -DBL_MAX;

    const double *rhs = lp.getRightHandSide();
    int nBin = 0, nInt = 0;
    int nRPart = 0, nrBFlow = 0, nRPack = 0;

    char allIntegral = 1;
    const CoinPackedMatrix *cpmRow =  lp.getMatrixByRow();
    for ( int row=0 ; (row<lp.getNumRows()) ; ++row )
    {
        const int nzRow = cpmRow->getVectorLengths()[row];
        const CoinBigIndex *starts = cpmRow->getVectorStarts();
        const int *ridx = cpmRow->getIndices() + starts[row];
        ...
        int nBinRow = 0;
        double mincrow = DBL_MAX;
        double maxcrow = -DBL_MAX;
        int nPos=0, nNeg=0;
        int nIntegralCoefRow = 0;

        for ( int i=0 ; (i<nzRow) ; ++i )
        {
            double rround = floor(rcoef[i]+0.5);
            if (fabs(rround-rcoef[i])>1e-8)
                allIntegral = 0;
            else
                ++nIntegralCoefRow;

            mincrow = min(mincrow, rcoef[i]);
            maxcrow = max(maxcrow, rcoef[i]);
            ...
            if (lp.isBinary(ridx[i]))
                nBinRow++;
        }
        minAij = std::min( minAij, mincrow );
        maxAij = std::max( maxAij, maxcrow );

        if (nBinRow==nzRow)
        {
            // row with binary variables

            const char rsense = lp.getRowSense()[row];
            switch (rsense)
            {
                case 'E':
                {
                    ...
                    if ( (fabs(rhs[row]-1.0)<=1e-9) && (mincrow>=-1.00001) && (maxcrow<=1.00001) && (nIntegralCoefRow==nzRow) )
                    {
                        // partitioning
                        nRPart++;
                    }
                    ...
                    {
                        // binary flow
                        if ( (fabs(rhs[row])<=1e-9) && (nPos) && (nNeg) )
                        {
                            nrBFlow++;
                        }
                    }
                ...
                case 'L':
                {
                    if ( (fabs(rhs[row]-1.0)<=1e-9) && (mincrow>=0.99999) && (maxcrow<=1.00001) && (nIntegralCoefRow==nzRow) )
                    {
                        nRPack++;
                    }
                    break;
        ...
        minR = min( minR, rhs[row] );
        maxR = max( maxR, rhs[row] );
        {
            double rround = floor(rhs[row]+0.5);
            if (fabs(rround-rhs[row])>1e-8)
                allIntegral = 0;
        }
    }
    ...
    for ( int col=0 ; (col<lp.getNumCols()) ; ++col )
    {
        if (fabs(obj[col])>=1e-8)
        {
            mino = min(mino, obj[col]);
            maxo = max(maxo, obj[col]);
        }
    }
    ...
    if (nRPart>264)
    {
        if (nInt>253)
        {
            if (nrBFlow>1230)
            {
                ...
                clpOptions.setSpecialOption(1, 2, 100); // idiot
                return;
            }
            else
            {
                lp.getModelPtr()->setPerturbation(61);
                clpOptions.setSolveType( ClpSolve::useDual );
                clpOptions.setSpecialOption(0, 1, 1); // crash
                return;
            }
        }
        else
        {
            ...
            {
                clpOptions.setSolveType( ClpSolve::usePrimal );
                clpOptions.setSpecialOption(1, 2, 5); // idiot
                return;
            }
        }
    }
    else
    {
        if (nRPart>60)
        {
            ...
            {
                ClpPEDualRowSteepest p(0.5);
                lp.getModelPtr()->setDualRowPivotAlgorithm(p);
                return;
            }
        }
        else
